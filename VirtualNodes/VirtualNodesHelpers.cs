...
namespace VirtualNodes
{
    ...
    {
        /// <summary>
        ///     Checks a given node's name against a potential duplicate name. If the name is the same, followed by a space, a
        ///     parenthesis and a number, then this is a duplicate name.
        /// </summary>
        /// <param name="potentialDuplicateName">The name to check against</param>
        /// <param name="currNodeName">The given node's name</param>
        /// <returns>
        ///     True if the potential duplicate name is same with the current node's name followed by a parenthesis with a
        ///     number
        /// </returns>
        public static bool MatchDuplicateName(string potentialDuplicateName, string currNodeName)
        {
            ...
            return rgName.IsMatch(potentialDuplicateName) &&
                   rgName.Replace(potentialDuplicateName, "$1").Equals(currNodeName);
        }
        ...
        public static bool IsVirtualNode(this IPublishedContent item)
        {
            ...
            foreach (var rule in VirtualNodesRuleManager.Instance.Rules)
