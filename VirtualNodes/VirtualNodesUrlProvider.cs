...
namespace VirtualNodes
{
    public class VirtualNodesUrlProvider : DefaultUrlProvider
    {
        ...
        public VirtualNodesUrlProvider(IRequestHandlerSection requestSettings, ILogger logger,
            ...
            string culture, Uri current)
        {
            ...
            return hasVirtualNodeInPath
                ? ConstructUrl(umbracoContext, content, mode, culture, current)
                : base.GetUrl(umbracoContext, content, mode, culture, current);
        }


        private UrlInfo ConstructUrl(UmbracoContext umbracoContext, IPublishedContent content, UrlMode mode,
            string culture, Uri current)
        {
            ...
            if (urlText.StartsWith("http", StringComparison.Ordinal))
            {
            ...
            if (urlText.EndsWith("/", StringComparison.Ordinal)) urlText = urlText.Substring(0, urlText.Length - 1);

            if (urlText.StartsWith("/", StringComparison.Ordinal)) urlText = urlText.Substring(1, urlText.Length - 1);

            // Now split the url. We should have as many elements as those in pathIds.
            var urlParts = urlText.Split('/').Reverse().ToArray();
            ...
            var finalUrl = string.Join("/", urlParts.Reverse().Where(x => x != "").ToArray());

            // Just in case - check if there are trailing and leading slashes and add them if not
            if (!finalUrl.EndsWith("/", StringComparison.Ordinal) && _requestSettings.AddTrailingSlash) finalUrl += "/";

            if (!finalUrl.StartsWith("/", StringComparison.Ordinal)) finalUrl = "/" + finalUrl;
